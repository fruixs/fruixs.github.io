...
class MenubarNavigation {
	constructor(domNode) {
		...
		console.log("linkURL", linkURL, linkTitle)
		...
		this.menuitems.forEach((item) => {
			if (item.href === linkURL) {
				...
				while (pathNode) {
					...
					pathNode.title = '현재 페이지 링크를 포함하고 있습니다';
	...
	onKeydown(event) {
		...
		/* 
		이벤트 흐름 제어 정지
		// a link가 Hash 링크(#링크)일 경우 체크
		const re = /^(#[A-Za-z0-9-._~:/?#[\]@!$&'()*+,;=%]*)/;
		//console.log(tgt.getAttribute('href').match(re)[0] == '', tgt.getAttribute('href'))
		//console.log(re.exec(tgt.getAttribute('href'))[0] == '')
		//re.exec(tgt.getAttribute('href'))[0] == '' ? location.href = tgt.getAttribute('href') : '';
		console.log(re.test(tgt.getAttribute('href')), 'test()')
		re.test(tgt.getAttribute('href')) ? '' : location.href = tgt.getAttribute('href');
